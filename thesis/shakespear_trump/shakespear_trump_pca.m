...
pcaRes = LIBRA.robpca(embeddings, 'k', 20, 'kmax', 20, 'alpha', 0.7, 'mcd', 0, 'plots', 0);
scores = pcaRes.T;
...
function stats = runComparison(NameValueArgs)
    arguments
        ...
        NameValueArgs.matryoshkaDim (1,1) double {mustBePositive, mustBeInteger} = Inf
        NameValueArgs.principalComponents (1,1) double {mustBePositive, mustBeInteger} = 20
    end
    ...
    for i = start:iter
        ...
        pcaRes = LIBRA.robpca(data, 'k', NameValueArgs.principalComponents, 'kmax', NameValueArgs.principalComponents, 'alpha', alpha, 'mcd', 0, 'plots', 0);
        scores = pcaRes.T;
        ...
        e = evaluation(scores, labels, alpha, solution, Estimators={'lof' 'iforest'});
